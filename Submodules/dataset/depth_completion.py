...
from Submodules.utils import vis_utils, depth_map_utils
...
def main():
    ...
    # Test set
    input_depth_root_dir = os.path.expanduser(
        'kitti/train')
    # data_split = 'test'
    ...
    if len(output_list) > 0:
        ...
        last_output_index = -1
    output_depth_root_dir = os.path.join(outputs_dir, '{}_{:03d}'.format(
        output_folder_prefix, last_output_index + 1))

    if save_output:
        if not os.path.exists(output_depth_root_dir):
            os.makedirs(output_depth_root_dir)
        else:
            raise FileExistsError('Already exists!')
        print('Output dir:', output_depth_root_dir)
